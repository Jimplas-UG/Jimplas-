...
/** Sim desk notional when MT5 is not connected. */
export const SIM_DESK_EQUITY = 50000;
...
export function structuralSlPipsFromTrade(trade, pip) {
  ...
  const p = pip > 0 ? pip : defaultBilshenzConfig.pipSize;
...
export function journalSizingSlPips(structuralSlPips, cfg) {
  const fixed = cfg?.journalSizingSlPips ?? defaultBilshenzConfig.journalSizingSlPips ?? 0;
...
export function computeRiskSizing(p) {
  ...
  const pct = p.riskPct > 0 && Number.isFinite(p.riskPct) ? p.riskPct : defaultBilshenzConfig.riskPctAtrNormal;
...
export function sizingForTrade(trade, cfg, equity, riskPct) {
  const pip = cfg?.pipSize ?? defaultBilshenzConfig.pipSize;
  ...
  const base = computeRiskSizing({
    ...
    riskPct,
    slPips: sizingSlPips,
    usdPerPipPerLot: cfg?.simUsdPerEnginePip ?? defaultBilshenzConfig.simUsdPerEnginePip,
  });
  let rewardUsd = 0;
...
export function journalClosedUsd(rows, cfg, equity, riskPct) {
  const pip = cfg?.pipSize ?? defaultBilshenzConfig.pipSize;
  const pipUsd = cfg?.simUsdPerEnginePip ?? defaultBilshenzConfig.simUsdPerEnginePip;
  let total = 0;
  for (const r of rows) {
