...
import {
  publicChannelStates,
  publicExecuteMessage,
  publicRiskLevel,
  publicSetupPill,
  publicSignalSide,
  publicStatusLine,
} from './security/publicLabels';
import {
  clearProfilePhoto,
...
function displayCfg(engCfg) {
  if (engCfg) return engCfg;
  return { pipSize: DISPLAY_PIP_SIZE, maxDailyTrades: 5, beOffset: 1.2, journalSizingSlPips: 0, simUsdPerEnginePip: 12.5 };
}

/** S/R view for UI — production snapshots omit engine sr/range blobs. */
function mapSrForUi(snap, livePrice, C) {
  if (!snap?.sr || !snap?.range || snap._sanitized) {
    return sanitizeSrView({
      currentPrice: livePrice,
      pos: '—',
      verdictVal: snap?._public?.signal ?? '—',
      verdictSub: snap?._public?.statusLine ?? 'Desk active',
      verdictLabelColor: C.dim,
      verdictValColor: C.text,
      verdictBorder: C.border,
  ...
  return sanitizeSrView(mapSrFromEngine(snap, livePrice, C));
}
...
function buildGmAlertRows(r, nfpBlackout, newsActive, C) {
  if (!r) return [];
  if (r.riskLevel != null && r.atrPips == null && r.chopZone === undefined) {
    const c = r.riskLevel === 'HIGH' ? C.red : r.riskLevel === 'MEDIUM' ? C.amber : C.teal;
    return [{ color: c, text: `DESK RISK: ${r.riskLevel}` }];
  }
  const rows = [];
  rows.push({
...
/** Risk tier for sizing display — exact thresholds only in dev INTEL mode. */
function effectiveRiskPctFromEngine(geoRisk, atrPips, cfg) {
  if (!SHOW_STRATEGY_INTEL) {
    return geoRisk === 'HIGH' ? 0.5 : geoRisk === 'MEDIUM' ? 0.7 : 1;
  }
  const d = displayCfg(cfg);
  let pct = d.riskPctAtrNormal ?? 1;
  if (atrPips != null && Number.isFinite(atrPips)) {
    if (atrPips >= 100) pct = d.riskPctAtrCrisis ?? 0.5;
    else if (atrPips >= 50) pct = d.riskPctAtrElevated ?? 0.7;
  }
  if (geoRisk === 'HIGH') pct = Math.min(pct, d.riskPctGeoHighCap ?? 0.5);
  return pct;
}

function deskChannelStates(signals) {
  if (!SHOW_STRATEGY_INTEL) return publicChannelStates(signals);
...
function deskStrategyModeLine(cfg) {
  if (!SHOW_STRATEGY_INTEL) return 'Desk protocol active';
  const d = displayCfg(cfg);
  const tp =
    d.useLegacyTpClampOnly && d.tp1MinRewardPips != null
...
function activeSetupGrabLbl(signals, trade) {
  if (!SHOW_STRATEGY_INTEL) return publicSetupPill(trade);
...
function wickStoryLines(wick, pipSize) {
  if (!SHOW_STRATEGY_INTEL || !wick) {
    return { main: 'Market structure scan in progress', sub: '—' };
  }
  const pip = pipSize > 0 ? pipSize : DISPLAY_PIP_SIZE;
  const rngP = (wick.candleRange / pip).toFixed(0);
  const bodyPct = (wick.bodyRatio * 100).toFixed(0);
  let main = 'M30 wick scan — awaiting flip / rejection stack';
  if (wick.jimplasFlipBuy) main = 'WICK CREATED ✓ — flip BUY';
  else if (wick.jimplasFlipSell) main = 'WICK CREATED ✓ — flip SELL';
  else if (wick.isValidRejection) main = 'WICK REJECTION ✓';
  const sub = `M30 · range ${rngP}p · body ${bodyPct}%`;
  return { main, sub };
}
...
function LeftColumn({ sr, dxy }) {
  ...
  const geoCap = cfg?.riskPctGeoHighCap ?? 0.5;
  const geoRule =
    risk?.geoHigh
  ...
  return (
    <View style={styles.leftCol}>
      ...
      <Panel
        ...
        head={{
          title: SHOW_STRATEGY_INTEL ? 'Jimplas Setups' : 'Setup channels',
          badge: SHOW_STRATEGY_INTEL ? 'P1 · P2 · P3' : 'A · B · C',
          titleColor: C.goldL,
          badgeColor: C.gold,
        }}
      >
        <DxyRow
          l={SHOW_STRATEGY_INTEL ? 'P1 Breakout + retest' : 'Channel A'}
          v={jLive.p1 ?? jLive.a ?? '—'}
          vc={(jLive.p1 ?? jLive.a) === 'LIVE' ? C.green : C.dim}
        />
        <DxyRow
          l={SHOW_STRATEGY_INTEL ? 'P2 Wick fill' : 'Channel B'}
          v={jLive.p2 ?? jLive.b ?? '—'}
          vc={(jLive.p2 ?? jLive.b) === 'LIVE' ? C.teal : C.dim}
        />
        <DxyRow
          l={SHOW_STRATEGY_INTEL ? 'P3 Session impulse' : 'Channel C'}
          v={jLive.p3 ?? jLive.c ?? '—'}
          vc={(jLive.p3 ?? jLive.c) === 'LIVE' ? C.amber : C.dim}
        />
        <Text style={styles.geoSub}>{deskStrategyModeLine(cfgJ)}</Text>
      </Panel>
...
function TradeSetupChip({ label, state }) {
  const { colors: C, styles } = useBilshenzTheme();
  const live = state === 'LIVE';
  return (
    <View style={[styles.tradeSetupChip, live && styles.tradeSetupChipLive]}>
      <Text style={styles.tradeSetupChipLbl}>{label}</Text>
      <Text style={[styles.tradeSetupChipVal, { color: live ? C.green : C.dim }]}>{state}</Text>
    </View>
  );
}

function TradeSignalCompact({
  sideLbl,
  sideColor,
  trade,
  sessionBits,
  ...
  const sideBadgeBorder = hasSide
    ? trade.side === 'BUY'
      ? 'rgba(0,230,118,0.45)'
      : 'rgba(255,61,87,0.45)'
    : 'rgba(255,255,255,0.12)';
  const sideSub = hasSide
    ? SHOW_STRATEGY_INTEL
      ? `${trade.setup ?? '—'} · LIVE`
      : 'Signal live'
    : SHOW_STRATEGY_INTEL
      ? 'P1 → P2 → P3'
      : 'Monitoring';
  const confPct = engineTrade?.confidencePct;
  const confStr = confPct != null && Number.isFinite(confPct) ? `${confPct.toFixed(1)}%` : '—';
  const execLabel = !engineReady
    ? '⏳ SYNC…'
    : execBusy
      ? '⏳ PROCESSING…'
      : tradeCount >= tradeCap
  ...
  return (
    <View style={styles.sigInnerStack}>
      <Row style={styles.tradeSigHeader}>
        <View style={[styles.tradeSigSideBadge, { backgroundColor: sideBadgeBg, borderColor: sideBadgeBorder }]}>
          <Text style={[styles.tradeSigSideTxt, { color: sideColor }]}>{sideLbl}</Text>
          <Text style={styles.tradeSigSideSub}>{sideSub}</Text>
        </View>
        <View style={styles.tradeSigSessChip}>
          <Text style={styles.tradeSigSessLbl}>SESSION</Text>
          <Text style={styles.tradeSigSessTxt} numberOfLines={2}>
            {sessionBits.sessLabel}
          </Text>
        </View>
      </Row>
      ...
      <Row style={styles.tradeSigSetupRow}>
        <TradeSetupChip label={SHOW_STRATEGY_INTEL ? 'P1' : 'A'} state={jLive.p1 ?? jLive.a ?? 'SCAN'} />
        <TradeSetupChip label={SHOW_STRATEGY_INTEL ? 'P2' : 'B'} state={jLive.p2 ?? jLive.b ?? 'SCAN'} />
        <TradeSetupChip label={SHOW_STRATEGY_INTEL ? 'P3' : 'C'} state={jLive.p3 ?? jLive.c ?? 'SCAN'} />
      </Row>

      <View style={styles.tradeSigEngineBox}>
        <Row style={styles.tradeSigConfRow}>
          <Text style={styles.tradeSigConfLbl}>{SHOW_STRATEGY_INTEL ? '⬡ ENGINE GATES' : 'CONFIDENCE'}</Text>
          <Text style={styles.tradeSigConfPct}>{confStr}</Text>
        </Row>
        <Text style={styles.tradeSigReason}>
          {engineTrade?.statusLine ?? publicStatusLine(trade)}
        </Text>
        {engineTrade?.m15EarlyExit ? (
          <Text style={styles.tradeSigReason} numberOfLines={2}>
            ⚠ {engineTrade.m15EarlyExit.message} · exit ≈ {engineTrade.m15EarlyExit.exitPrice.toFixed(2)}
          </Text>
        ) : null}
        <Text style={styles.tradeSigWick} numberOfLines={2}>
          {wickLines.main}
        </Text>
      </View>
      ...
      <Row style={styles.sigBtnRow}>
        <TouchableOpacity
          ...
          <Text style={styles.execBtnTxtCompact}>{execLabel}</Text>
        </TouchableOpacity>
    ...
    trade?.side && (cfg?.journalSizingSlPips ?? 0) > 0 && slPipsE > sizingSlPips + 0.05
      ...
      : trade?.side === 'SELL'
        ...
        : trade?.side === 'BUY'
          ? 'Below entry − buffer'
          : 'Structure + buffer';
  const grabLbl = activeSetupGrabLbl(snap?.signals, trade);
  const stratModeLbl = deskStrategyModeLine(cfg);
    ...
    <ScrollView
      ...
      <View style={styles.psSettingsCard}>
        ...
        <Row style={styles.psToggleRow}>
          ...
          <Switch
            value={telegramNotifyEnabled}
            onValueChange={onTelegramNotifyEnabledChange}
            trackColor={{ false: C.border, true: 'rgba(34,171,226,0.45)' }}
            thumbColor={telegramNotifyEnabled ? C.blue : C.dim2}
            style={{ transform: [{ scaleX: 1.05 }, { scaleY: 1.05 }] }}
          />
        </Row>
        <Text style={styles.psRowLabel}>TELEGRAM RELAY URL</Text>
        <TextInput
          value={telegramRelayUrl}
          onChangeText={onTelegramRelayUrlChange}
          placeholder="https://your-worker.workers.dev/telegram"
          placeholderTextColor={C.dim2}
          autoCapitalize="none"
          autoCorrect={false}
          style={styles.psBrokerInput}
        />
        <Text style={[styles.psRowLabel, { marginTop: 8 }]}>RELAY SECRET (OPTIONAL BEARER)</Text>
        <TextInput
          value={telegramRelaySecret}
          onChangeText={onTelegramRelaySecretChange}
          placeholder="Shared secret Worker checks"
          placeholderTextColor={C.dim2}
          autoCapitalize="none"
          autoCorrect={false}
          secureTextEntry
  ...
  return (
    <View style={styles.ghWrap}>
      ...
      <View style={[styles.ghQuote, { marginHorizontal: pad }]}>
        ...
        <Text style={styles.ghQuoteTxt}>
          One protocol. Fifteen gates. Execute only when the desk agrees.
        </Text>
...
function AppContent({ onEngineReady }) {
  ...
  const [telegramRelayUrl, setTelegramRelayUrl] = useState('');
  const [telegramRelaySecret, setTelegramRelaySecret] = useState('');
    ...
    (async () => {
      try {
        const pairs = await AsyncStorage.multiGet([
          STORAGE_BROKER_HOOK_URL,
          STORAGE_TELEGRAM_RELAY_URL,
        ...
        if (m[STORAGE_BROKER_HOOK_URL]) setBrokerWebhookUrl(m[STORAGE_BROKER_HOOK_URL] ?? '');
  ...
  useEffect(() => {
    AsyncStorage.setItem(STORAGE_TELEGRAM_RELAY_URL, telegramRelayUrl).catch(() => {});
  }, [telegramRelayUrl]);

  useEffect(() => {
    AsyncStorage.setItem(STORAGE_TELEGRAM_RELAY_SECRET, telegramRelaySecret).catch(() => {});
  }, [telegramRelaySecret]);

  useEffect(() => {
    AsyncStorage.setItem(STORAGE_TELEGRAM_NOTIFY, telegramNotifyEnabled ? '1' : '0').catch(() => {});
  }, [telegramNotifyEnabled]);
  ...
  const telegramNotifyBarRef = useRef(null);

  useEffect(() => {
    if (!telegramNotifyEnabled) telegramNotifyBarRef.current = null;
  }, [telegramNotifyEnabled]);
  ...
  const sendTelegramEligibleIfNeeded = useCallback(
    async (intent, lotsEstimated, barT) => {
      if (!telegramNotifyEnabled || !telegramRelayUrl.trim() || !intent || barT == null) return;
      if (telegramNotifyBarRef.current === barT) return;
      telegramNotifyBarRef.current = barT;
      const r = await postTelegramSignalRelay(intent, {
        relayUrl: telegramRelayUrl.trim(),
        relaySecret: telegramRelaySecret.trim() || undefined,
        lotsEstimated,
      });
      if (!r.ok) {
        telegramNotifyBarRef.current = null;
        setLastBrokerMsg(`Telegram relay: ${r.snippet}`);
      }
    },
  ...
  const sr = useMemo(() => mapSrForUi(bzSnapshot, chartPrice, C), [bzSnapshot, chartPrice, C]);
  ...
  const flowNodes = useMemo(() => {
    if (!SHOW_STRATEGY_INTEL || !bzSnapshot.gates) return [];
    const ec = bilshenzEngine.cfg ?? displayCfg(null);
            ...
            {isWide ? (
              ...
              <View style={[styles.mobileTabBody, styles.psTabBody, { paddingHorizontal: 0 }]}>
                <ProfileTab
                  ...
                  telegramRelayUrl={telegramRelayUrl}
                  onTelegramRelayUrlChange={setTelegramRelayUrl}
                  telegramRelaySecret={telegramRelaySecret}
                  onTelegramRelaySecretChange={setTelegramRelaySecret}
