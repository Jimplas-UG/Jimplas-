...
Telegram group alerts — two supported paths
============================================
Never put your Telegram BOT_TOKEN inside the Expo app.

A) Profile → “TELEGRAM RELAY URL” (works with Python MT5 API from phone)
   - Create a bot with @BotFather, add it to your group, get chat_id
     (GET https://api.telegram.org/bot<TOKEN>/getUpdates after a test message).
   - Deploy a tiny HTTPS endpoint (Cloudflare Worker, etc.) that:
       1) Verifies optional Authorization: Bearer <shared secret>
       2) Reads JSON body (see myapp/broker/telegramRelay.ts payload)
       3) POSTs to https://api.telegram.org/bot<TOKEN>/sendMessage
          with chat_id and text = body.text (or format from body.intent)
   - App toggles “TELEGRAM — ELIGIBLE SIGNALS” and sends the same moment
     as a gated manual EXEC or auto-execute (one ping per M30 bar).

B) Webhook bridge only (mt5/bridge-server.mjs)
   - Set on the PC running the bridge:
       set TELEGRAM_BOT_TOKEN=...
       set TELEGRAM_CHAT_ID=...   (group id often negative)
   - When /webhook queues a TRADE line, the bridge also calls sendMessage.
   - Optional: use A) relay from the app as well — avoid duplicate groups
     unless you want both.

Optional build-time defaults (dev only):
  EXPO_PUBLIC_TELEGRAM_NOTIFY_URL
